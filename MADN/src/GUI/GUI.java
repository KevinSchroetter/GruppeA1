...
//		windowScroller.setVisible(true);
	...
	private void setEventhandler() {
		this.myHandler = new Eventhandler(naviMap, labelMap, imagesMap, gifMap,
				sMap, startMap, endMap, dateiGrabber, mitte, this);
		myHandler.addGUI(frame);
		/*
		 * for(java.util.Map.Entry<String, JButton> entry : sMap.entrySet()) {
		 * JButton value = entry.getValue(); mitte.add(value);
		 * value.addActionListener(myHandler); } for(java.util.Map.Entry<String,
		 * JButton> entry : startMap.entrySet()) { JButton value =
		 * entry.getValue(); mitte.add(value);
		 * value.addActionListener(myHandler); } for(java.util.Map.Entry<String,
		 * JButton> entry : endMap.entrySet()) { JButton value =
		 * entry.getValue(); mitte.add(value);
	...
	private void addIconToImagesMap(String imageName, String path, int width,
			int length) {
		ImageIcon icon = new ImageIcon(path);
		icon.setImage(icon.getImage().getScaledInstance(width, length,
				Image.SCALE_SMOOTH));
		imagesMap.put(imageName, icon);
		meep.setProgress(2);
	}

	private void addIconToGifMap(String imageName, String path, int width,
			int length) {
		ImageIcon icon = new ImageIcon(path);
		icon.setImage(icon.getImage().getScaledInstance(width, length,
				Image.SCALE_FAST));
	...
	private void setFieldBackgroundImage(String image) {
		if (labelMap.isEmpty())
			throw new RuntimeException("labelMap leer");
	...
	private void initNaviButtonsByLanguage(int index) {
		...
		if (index < 0 || index > ButtonsNavi.values().length)
			...
			JButton diceGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("roll").ordinal()));
			naviMap.put("diceGame", diceGame);
			Dimension d = new Dimension(language.get(0)
					.get(ButtonsNavi.valueOf("roll").ordinal()).getIconWidth(),
					language.get(0).get(1).getIconHeight());
			diceGame.setPreferredSize(d);
			diceGame.setEnabled(false);
			;
			JButton startGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("start").ordinal()));
			startGame.setPreferredSize(d);
			naviMap.put("startGame", startGame);
			JButton endGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("end").ordinal()));
			endGame.setPreferredSize(d);
			endGame.setSize(130, 80);
			naviMap.put("endGame", endGame);
			JButton newGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("newGame").ordinal()));
			newGame.setPreferredSize(d);
			naviMap.put("newGame", newGame);
			newGame.setEnabled(false);

			JButton saveGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("save").ordinal()));
			saveGame.setPreferredSize(d);
			naviMap.put("saveGame", saveGame);
			saveGame.setEnabled(false);

			JButton loadGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("load").ordinal()));
			loadGame.setPreferredSize(d);
			naviMap.put("loadGame", loadGame);

			JButton sendGame = new JButton(language.get(index).get(
					ButtonsNavi.valueOf("send").ordinal()));
	...
	private void initStartFieldButtons() {
		JButton s1Rot = new JButton("S1ROT");
		startMap.put("S1ROT", s1Rot);
		JButton s2Rot = new JButton("S2ROT");
		startMap.put("S2ROT", s2Rot);
		JButton s3Rot = new JButton("S3ROT");
		startMap.put("S3ROT", s3Rot);
		JButton s4Rot = new JButton("S4ROT");
		startMap.put("S4ROT", s4Rot);
		JButton s1Blau = new JButton("S1BLAU");
		startMap.put("S1BLAU", s1Blau);
		JButton s2Blau = new JButton("S2BLAU");
		startMap.put("S2BLAU", s2Blau);
		JButton s3Blau = new JButton("S3BLAU");
		startMap.put("S3BLAU", s3Blau);
		JButton s4Blau = new JButton("S4BLAU");
		startMap.put("S4BLAU", s4Blau);
		JButton s1Gruen = new JButton("S1GRUEN");
		startMap.put("S1GRUEN", s1Gruen);
		JButton s2Gruen = new JButton("S2GRUEN");
		startMap.put("S2GRUEN", s2Gruen);
		JButton s3Gruen = new JButton("S3GRUEN");
		startMap.put("S3GRUEN", s3Gruen);
		JButton s4Gruen = new JButton("S4GRUEN");
		startMap.put("S4GRUEN", s4Gruen);
		JButton s1Gelb = new JButton("S1GELB");
		startMap.put("S1GELB", s1Gelb);
		JButton s2Gelb = new JButton("S2GELB");
		startMap.put("S2GELB", s2Gelb);
		JButton s3Gelb = new JButton("S3GELB");
		startMap.put("S3GELB", s3Gelb);
		JButton s4Gelb = new JButton("S4GELB");
		startMap.put("S4GELB", s4Gelb);
	}

	private void initEndFieldButtons() {
		JButton e1Rot = new JButton("E1ROT");
		endMap.put("E1ROT", e1Rot);
		JButton e2Rot = new JButton("E2ROT");
		endMap.put("E2ROT", e2Rot);
		JButton e3Rot = new JButton("E3ROT");
		endMap.put("E3ROT", e3Rot);
		JButton e4Rot = new JButton("E4ROT");
		endMap.put("E4ROT", e4Rot);
		JButton e1Blau = new JButton("E1BLAU");
		endMap.put("E1BLAU", e1Blau);
		JButton e2Blau = new JButton("E2BLAU");
		endMap.put("E2BLAU", e2Blau);
		JButton e3Blau = new JButton("E3BLAU");
		endMap.put("E3BLAU", e3Blau);
		JButton e4Blau = new JButton("E4BLAU");
		endMap.put("E4BLAU", e4Blau);
		JButton e1Gruen = new JButton("E1GRUEN");
		endMap.put("E1GRUEN", e1Gruen);
		JButton e2Gruen = new JButton("E2GRUEN");
		endMap.put("E2GRUEN", e2Gruen);
		JButton e3Gruen = new JButton("E3GRUEN");
		endMap.put("E3GRUEN", e3Gruen);
		JButton e4Gruen = new JButton("E4GRUEN");
		endMap.put("E4GRUEN", e4Gruen);
		JButton e1Gelb = new JButton("E1GELB");
		endMap.put("E1GELB", e1Gelb);
		JButton e2Gelb = new JButton("E2GELB");
		endMap.put("E2GELB", e2Gelb);
		JButton e3Gelb = new JButton("E3GELB");
		endMap.put("E3GELB", e3Gelb);
		JButton e4Gelb = new JButton("E4GELB");
		endMap.put("E4GELB", e4Gelb);
	}
	...
	private void setFieldPosition(JButton field, int x, int y, int width,
	...
	private void setStandardFieldPositions(int width, int height) {
		...
		setFieldPosition(sMap.get("1"), 33, 290, width, height, false); // fixed
																		// y
		setFieldPosition(sMap.get("2"), 120, 290, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("3"), 205, 290, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("4"), 284, 290, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("5"), 371, 290, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("6"), 371, 213, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("7"), 371, 138, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("8"), 371, 63, width, height, false); // fixed
																		// y
		setFieldPosition(sMap.get("9"), 371, -8, width, height, false); // fixed
																		// y
		setFieldPosition(sMap.get("10"), 443, -8, width, height, false); // fixed
																			// y
																			// +35
		// StandardFelder fuer Spieler BLAU
		setFieldPosition(sMap.get("11"), 521, -8, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("12"), 521, 63, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("13"), 521, 138, width, height, false); // fixed
																			// y
																			// +33
		setFieldPosition(sMap.get("14"), 521, 213, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("15"), 521, 293, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("16"), 604, 293, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("17"), 689, 293, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("18"), 772, 293, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("19"), 854, 293, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("20"), 854, 371, width, height, false); // fixed
																			// y
		// StandardFelder fuer Spieler GRUEN
		setFieldPosition(sMap.get("21"), 852, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("22"), 772, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("23"), 689, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("24"), 604, 447, width, height, false); // fixed
																			// y
																			// +20?
		setFieldPosition(sMap.get("25"), 521, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("26"), 521, 523, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("27"), 521, 598, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("28"), 521, 673, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("29"), 521, 748, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("30"), 443, 748, width, height, false); // fixed
																			// y
		// StandardFelder fuer Spieler GELB
		setFieldPosition(sMap.get("31"), 370, 748, width, height, false); // fixed
																			// y
		...
		setFieldPosition(sMap.get("35"), 370, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("36"), 284, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("37"), 199, 445, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("38"), 117, 445, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("39"), 35, 447, width, height, false); // fixed
																			// y
		setFieldPosition(sMap.get("40"), 35, 370, width, height, false); // fixed
																			// y
	}
